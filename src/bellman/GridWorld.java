	...
	private HashMap<Integer,ArrayList<double[]>> computeTrans(String act) {
		...
		for(int i = 0; i < size_x; i++){
			for(int j = 0; j < size_y; j++){
				...
				double[] sPrime = new double[2];
				
				int[] stateShift = getDirNeighbor(act);
				sPrime[0] = GridToState((i + stateShift[0] + size_x)%size_x, (j + stateShift[1] + size_y)%size_y);
				sPrime[1] = 1; //Ici une action n'a qu'une seule issue, on met donc 1
				
				reachableStates.add(sPrime);
